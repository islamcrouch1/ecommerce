            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($currencies->count() > 0 && $currencies[0]->trashed())
                            {{ __('currencies') . ' ' . __('trash') }}
                        @else
                            {{ __('currencies') }}
                        @endif
                    </h5>
                ...
                <div class="col-8 col-sm-auto text-end ps-2">
                    ...
                    <div id="table-customers-replace-element">
                        ...
                        @if (auth()->user()->hasPermission('currencies-create'))
                            <a href="{{ route('currencies.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        <a href="{{ route('currencies.trashed') }}" class="btn btn-falcon-default btn-sm"
                        ...
                        <tbody class="list" id="table-customers-body">
                            @foreach ($currencies as $currency)
                                <tr class="btn-reveal-trigger">
                                        ...
                                        <div class="d-flex d-flex align-items-center">
                                            ...
                                            <div class="flex-1">
                                                <h5 class="mb-0 fs--1">
                                                    ...
                                                    @if ($currency->is_default == 'on')
                                                        <span class='badge badge-soft-success'>{{ __('default') }}</span>
                                                    @endif
                                    ...
                                    <td class="phone align-middle white-space-nowrap py-2">{{ $currency->symbol }}</td>

                                    <td class="phone align-middle white-space-nowrap py-2">
                                        @if ($currency->status == 'on')
                                    ...
                                    <td class="joined align-middle py-2">{{ $currency->created_at }} <br>
                                        {{ interval($currency->created_at) }} </td>
                                    @if ($currency->trashed())
                                        <td class="joined align-middle py-2">{{ $currency->deleted_at }} <br>
                                            {{ interval($currency->deleted_at) }} </td>
                                    @endif
                                        ...
                                        <div class="dropdown font-sans-serif position-static">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end border py-0"
                                                ...
                                                <div class="bg-white py-2">
                                                    @if (
                                                        $currency->trashed() &&
                                                            auth()->user()->hasPermission('currencies-restore'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('currencies.restore', ['currency' => $currency->id]) }}">{{ __('Restore') }}</a>
                                                    @elseif(auth()->user()->hasPermission('currencies-update'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('currencies.edit', ['currency' => $currency->id]) }}">{{ __('Edit') }}</a>
                                                    @endif
                                                    @if (auth()->user()->hasPermission('currencies-delete') ||
                                                            auth()->user()->hasPermission('currencies-trash'))
                                                        <form method="POST"
                                                            action="{{ route('currencies.destroy', ['currency' => $currency->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $currency->trashed() ? __('Delete') : __('Trash ') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </div>
