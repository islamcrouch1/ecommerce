            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($warehouses->count() > 0 && $warehouses[0]->trashed())
                            {{ __('warehouses') . ' ' . __('trash') }}
                        @else
                            {{ __('warehouses') }}
                        @endif
                    </h5>
                ...
                <div class="col-8 col-sm-auto text-end ps-2">
                    ...
                    <div id="table-customers-replace-element">
                        ...
                        @if (auth()->user()->hasPermission('warehouses-create'))
                            <a href="{{ route('warehouses.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        <a href="{{ route('warehouses.trashed') }}" class="btn btn-falcon-default btn-sm"
                            ...
                            @foreach ($warehouses as $warehouse)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="joined align-middle py-2">{{ $warehouse->created_at }} <br>
                                        {{ interval($warehouse->created_at) }} </td>
                                    @if ($warehouse->trashed())
                                        <td class="joined align-middle py-2">{{ $warehouse->deleted_at }} <br>
                                            {{ interval($warehouse->deleted_at) }} </td>
                                    @endif
                                                    ...
                                                    @if (auth()->user()->hasPermission('warehouses-delete') ||
                                                        ...
                                                        <form method="POST"
                                                            ...
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $warehouse->trashed() ? __('Delete') : __('Trash ') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </div>
