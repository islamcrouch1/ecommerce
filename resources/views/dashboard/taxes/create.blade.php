        ...
        <div class="card-header">
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">{{ __('Add New') . ' ' . __('tax') }}</h5>
                </div>
            </div>
        </div>
                        ...
                        <form method="POST" action="{{ route('taxes.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input name="tax_rate" class="form-control @error('tax_rate') is-invalid @enderror"
                                    value="{{ old('tax_rate') }}" type="text" autocomplete="on" id="tax_rate"
                                    required />
                                @error('tax_rate')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                <button class="btn btn-primary d-block w-100 mt-3" type="submit"
                                    name="submit">{{ __('Add New') . ' ' . __('tax') }}</button>
                            </div>
                        </form>
