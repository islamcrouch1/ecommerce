            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($cities->count() > 0 && $cities[0]->trashed())
                            {{ __('cities trash') }}
                        @else
                            {{ __('cities') }}
                        @endif
                    ...
                    <div id="table-customers-replace-element">
                        ...
                        <form style="display: inline-block" action="">
                            ...
                            <div class="d-inline-block">
                                <select name="state_id" class="form-select form-select-sm sonoo-search"
                                    id="autoSizingSelect">
                                    <option value="" selected>{{ __('All States') }}</option>
                        ...
                        @if (auth()->user()->hasPermission('cities-create'))
                            <a href="{{ route('cities.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        <a href="{{ route('cities.trashed') }}" class="btn btn-falcon-default btn-sm" type="button"><span
                    ...
                    <table class="table table-sm table-striped fs--1 mb-0 overflow-hidden">
                        <thead class="bg-200 text-900">
                            <tr>
                                ...
                                <th class="sort pe-1 align-middle white-space-nowrap" data-sort="phone">{{ __('state') }}
                                </th>
                        ...
                        <tbody class="list" id="table-customers-body">
                            @foreach ($cities as $city)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="phone align-middle white-space-nowrap py-2">
                                        {{ app()->getLocale() == 'ar' ? $city->state->name_ar : $city->state->name_en }}
                                    </td>
                                    <td class="phone align-middle white-space-nowrap py-2">
                                    ...
                                    <td class="joined align-middle py-2">{{ $city->created_at }} <br>
                                        {{ interval($city->created_at) }} </td>
                                    @if ($city->trashed())
                                        <td class="joined align-middle py-2">{{ $city->deleted_at }} <br>
                                            {{ interval($city->deleted_at) }} </td>
                                    @endif
                                        ...
                                        <div class="dropdown font-sans-serif position-static">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end border py-0"
                                                ...
                                                <div class="bg-white py-2">
                                                    @if (
                                                        $city->trashed() &&
                                                            auth()->user()->hasPermission('cities-restore'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('cities.restore', ['city' => $city->id]) }}">{{ __('Restore') }}</a>
                                                    @elseif(auth()->user()->hasPermission('cities-update'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('cities.edit', ['city' => $city->id]) }}">{{ __('Edit') }}</a>
                                                    @endif
                                                    @if (auth()->user()->hasPermission('cities-delete') ||
                                                            auth()->user()->hasPermission('cities-trash'))
                                                        <form method="POST"
                                                            action="{{ route('cities.destroy', ['city' => $city->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $city->trashed() ? __('Delete') : __('Trash ') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </div>
