            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($offers->count() > 0 && $offers[0]->trashed())
                            {{ __('offers trash') }}
                        @else
                            {{ __('offers') }}
                        @endif
                ...
                <div class="col-8 col-sm-auto text-end ps-2">
                    ...
                    <div id="table-customers-replace-element">
                        ...
                        @if (auth()->user()->hasPermission('offers-create'))
                            <a href="{{ route('offers.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        <a href="{{ route('offers.trashed') }}" class="btn btn-falcon-default btn-sm" type="button"><span
                            ...
                            @foreach ($offers as $offer)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="phone align-middle white-space-nowrap py-2">
                                        {{ getName($offer->country) }}
                                    </td>

                                    <td class="joined align-middle py-2">{{ $offer->ended_at }} <br>
                                        {{ interval($offer->ended_at) }}</td>


                                    <td class="joined align-middle py-2">{{ $offer->created_at }} <br>
                                        {{ interval($offer->created_at) }} </td>
                                    @if ($offer->trashed())
                                        <td class="joined align-middle py-2">{{ $offer->deleted_at }} <br>
                                            {{ interval($offer->deleted_at) }} </td>
                                    @endif
                                        ...
                                        <div class="dropdown font-sans-serif position-static">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end border py-0"
                                                ...
                                                <div class="bg-white py-2">
                                                    @if (
                                                        $offer->trashed() &&
                                                            auth()->user()->hasPermission('offers-restore'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('offers.restore', ['offer' => $offer->id]) }}">{{ __('Restore') }}</a>
                                                    @elseif(auth()->user()->hasPermission('offers-update'))
                                                        <a class="dropdown-item"
                                                            href="{{ route('offers.edit', ['offer' => $offer->id]) }}">{{ __('Edit') }}</a>
                                                    @endif
                                                    @if (auth()->user()->hasPermission('offers-delete') ||
                                                            auth()->user()->hasPermission('offers-trash'))
                                                        <form method="POST"
                                                            action="{{ route('offers.destroy', ['offer' => $offer->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $offer->trashed() ? __('Delete') : __('Trash') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </div>
