            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($brands->count() > 0 && $brands[0]->trashed())
                            {{ __('Brands') . ' ' . __('trash') }}
                        @else
                            {{ __('Brands') }}
                        @endif
                ...
                <div class="col-8 col-sm-auto text-end ps-2">
                    ...
                    <div id="table-customers-replace-element">
                        ...
                        @if (auth()->user()->hasPermission('brands-create'))
                            <a href="{{ route('brands.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        <a href="{{ route('brands.trashed') }}" class="btn btn-falcon-default btn-sm" type="button"><span
                        ...
                        <tbody class="list" id="table-customers-body">
                            @foreach ($brands as $brand)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="name align-middle white-space-nowrap py-2">
                                        <div class="d-flex d-flex align-items-center">
                                            <div class="avatar avatar-xl me-2">
                                                <img class="rounded-circle" src="{{ asset($brand->media->path) }}"
                                                    alt="" />
                                            </div>
                                            <div class="flex-1">
                                                <h5 class="mb-0 fs--1">
                                    ...
                                    <td class="phone align-middle white-space-nowrap py-2">{{ $brand->brand_slug }}
                                    </td>
                                    <td class="phone align-middle white-space-nowrap py-2">
                                        {{ $brand->status == 'active' ? __('active') : __('inactive') }} </td>
                                    <td class="phone align-middle white-space-nowrap py-2">{{ $brand->sort_order }}</td>
                                    <td class="joined align-middle py-2">{{ $brand->created_at }} <br>
                                        {{ interval($brand->created_at) }} </td>
                                    @if ($brand->trashed())
                                        <td class="joined align-middle py-2">{{ $brand->deleted_at }} <br>
                                            {{ interval($brand->deleted_at) }} </td>
                                    @endif
                                                    ...
                                                    @if (auth()->user()->hasPermission('brands-delete') ||
                                                        ...
                                                        <form method="POST"
                                                            ...
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $brand->trashed() ? __('Delete') : __('Trash') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </div>
