            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">
                        @if ($attributes->count() > 0 && $attributes[0]->trashed())
                            {{ __('attributes') . ' ' . __('trash') }}
                        @else
                            {{ __('attributes') }}
                        @endif
                    </h5>
                ...
                <div class="col-8 col-sm-auto text-end ps-2">
                    ...
                    <div id="table-customers-replace-element">
                        ...
                        @if (auth()->user()->hasPermission('attributes-create'))
                            <a href="{{ route('attributes.create') }}" class="btn btn-falcon-default btn-sm"
                                type="button"><span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">{{ __('New') }}</span></a>
                        @endif
                        <a href="{{ route('attributes.trashed') }}" class="btn btn-falcon-default btn-sm"
                        ...
                        <tbody class="list" id="table-customers-body">
                            @foreach ($attributes as $attribute)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="joined align-middle py-2">{{ $attribute->created_at }} <br>
                                        {{ interval($attribute->created_at) }} </td>
                                    @if ($attribute->trashed())
                                        <td class="joined align-middle py-2">{{ $attribute->deleted_at }} <br>
                                            {{ interval($attribute->deleted_at) }} </td>
                                    @endif
                                                    ...
                                                    @if (auth()->user()->hasPermission('attributes-delete') ||
                                                        ...
                                                        <form method="POST"
                                                            ...
                                                            <button class="dropdown-item text-danger"
                                                                type="submit">{{ $attribute->trashed() ? __('Delete') : __('Trash ') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </div>
