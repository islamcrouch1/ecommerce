        ...
        <div class="card-header">
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">{{ __('edit') . ' ' . __('state') }}</h5>
                </div>
            </div>
        </div>
                        ...
                        <form method="POST" action="{{ route('states.update', ['state' => $state->id]) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <input name="name_ar" class="form-control @error('name_ar') is-invalid @enderror"
                                    value="{{ $state->name_ar }}" type="text" autocomplete="on" id="name_ar" autofocus
                                    required />
                                @error('name_ar')
                            ...
                            <div class="mb-3">
                                ...
                                <input name="name_en" class="form-control @error('name_en') is-invalid @enderror"
                                    value="{{ $state->name_en }}" type="text" autocomplete="on" id="name_en" autofocus
                                    required />
                                @error('name_en')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <input name="shipping_amount"
                                    ...
                                    value="{{ $state->shipping_amount }}" type="number" step="0.01" min="0"
                            ...
                            <div class="mb-3">
                                <button class="btn btn-primary d-block w-100 mt-3" type="submit"
                                    name="submit">{{ __('edit') . ' ' . __('state') }}</button>
                            </div>
                        </form>
